...
V7.1:
    GloVe 6B 100d + Mean Pooling + LinearSVC
"""
...
def encode_document(text, vectors, embedding_dim=100):
    """
    Encode one document using mean pooling.
    ...
    return np.mean(np.asarray(word_vectors), axis=0)


def encode_documents(texts, vectors, embedding_dim=100):
    """
    Encode multiple documents using mean pooling.

    Returns:
        embeddings
        number of documents with no known tokens
    """
...
def main():
    ...
    print()
    print("=" * 80)
    print("V7.1 - GloVe 100d + Mean Pooling + LinearSVC")
    print("=" * 80)
    ...
    X_test_embeddings, empty_test = (
        encode_documents(X_test, vectors)
    )

    test_encoding_time = (time.perf_counter() - start)
    ...
    for train_size in TRAIN_SIZES:
        ...
        X_train_embeddings, empty_train = (
            encode_documents(X_train, vectors)
        )
        ...
        results.append(
            {
                ...
                "Representation details": (
                    "GloVe 6B 100d + Mean Pooling"
                ),

                "Preprocessor": "None",
                "Variant": "Mean Pooling",
    ...
    print()
    print("Saved: results_V7_1_embeddings.csv")
